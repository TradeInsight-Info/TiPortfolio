...
`Weigh.BasedOnBeta` adjusts portfolio weights iteratively so that the resulting portfolio beta against a benchmark (e.g., SPY) reaches a specified target (typically 0 for beta-neutral). The residual or excess of the weight sum represents cash or leverage respectively — weights are not normalised.
