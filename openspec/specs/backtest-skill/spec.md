...
This capability defines the `backtest-skill` — a Claude Code skill that activates when users request portfolio backtesting or strategy evaluation. It maps natural language requests to `uvx tiportfolio` CLI commands (using uvx for ephemeral execution without installation), runs the backtest, and presents the results.
...
### Requirement: Ensure uvx is available
The skill SHALL verify that `uvx` is available before proceeding. All commands use `uvx tiportfolio` for ephemeral execution (no permanent install needed).

#### Scenario: uvx available
- **WHEN** `uvx --version` succeeds
- **THEN** the skill proceeds to building the CLI command using `uvx tiportfolio ...`

#### Scenario: uvx not available
- **WHEN** `uvx --version` fails (command not found)
- **THEN** the skill tells the user to install uv from https://docs.astral.sh/uv/
...
- **THEN** the skill builds: `uvx tiportfolio monthly --tickers QQQ,BIL,GLD --start 2019-01-01 --end 2024-12-31 --ratio equal`

#### Scenario: Custom ratio mapping
- **WHEN** user says "70/20/10 allocation of QQQ BIL GLD quarterly"
- **THEN** the skill builds: `uvx tiportfolio quarterly --tickers QQQ,BIL,GLD --ratio 0.7,0.2,0.1 --start <default> --end <default>`

#### Scenario: AIP mapping
- **WHEN** user says "monthly $1000 DCA into QQQ BIL GLD equal weight"
- **THEN** the skill builds: `uvx tiportfolio monthly --tickers QQQ,BIL,GLD --ratio equal --aip 1000 --start <default> --end <default>`

#### Scenario: Leverage mapping
- **WHEN** user says "compare 1x vs 1.5x vs 2x leverage on monthly QQQ BIL GLD"
- **THEN** the skill builds: `uvx tiportfolio monthly --tickers QQQ,BIL,GLD --ratio equal --leverage 1.0,1.5,2.0 --start <default> --end <default>`

### Requirement: Run command and present results
The skill SHALL execute the built `uvx tiportfolio` CLI command via Bash and present the results clearly to the user.
