...
Provides an AIP (Automatic Investment Plan) backtest function that simulates periodic cash injections into a portfolio over time. This enables users to model dollar-cost averaging strategies by injecting a fixed monthly amount into the portfolio on the last trading day of each calendar month, and running the full backtest engine with those contributions tracked.
