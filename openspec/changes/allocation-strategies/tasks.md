...
- [ ] 1.1 Write test in `tests/test_backtest.py`: spy that `get_target_weights` receives `prices_history` kwarg on rebalance dates (use a mock/spy strategy that captures context kwargs)
- [ ] 1.2 Update `run_backtest()` in `backtest.py`: on each rebalance call, pass `prices_history=prices_df.loc[:date]` in the context dict
- [ ] 1.3 Run existing tests to confirm `FixRatio` and `VixRegimeAllocation` are unaffected

## 2. VolatilityTargeting strategy

- [ ] 2.1 Write tests in `tests/test_allocation_vol_targeting.py`:
...
- [ ] 2.2 Implement `VolatilityTargeting` dataclass in `allocation.py`:
...
- [ ] 2.3 Export `VolatilityTargeting` from `src/tiportfolio/__init__.py`

## 3. DollarNeutral strategy

- [ ] 3.1 Write tests in `tests/test_allocation_dollar_neutral.py`:
...
- [ ] 3.2 Implement `DollarNeutral` dataclass in `allocation.py`:
...
- [ ] 3.3 Export `DollarNeutral` from `src/tiportfolio/__init__.py`

## 4. BetaNeutral strategy

- [ ] 4.1 Write tests in `tests/test_allocation_beta_neutral.py`:
...
- [ ] 4.2 Implement `BetaNeutral` dataclass in `allocation.py`:
...
- [ ] 4.3 Export `BetaNeutral` from `src/tiportfolio/__init__.py`

## 5. Final verification

- [ ] 5.1 Run full test suite (`uv run pytest`) and confirm all tests pass
- [ ] 5.2 Run `uv run mypy src/` and resolve any type errors in new code
