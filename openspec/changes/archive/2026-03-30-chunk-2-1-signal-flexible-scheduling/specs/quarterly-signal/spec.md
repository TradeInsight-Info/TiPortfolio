...
`Signal.Monthly` constructor SHALL rename `next_trading_day` → `closest_trading_day`, passing through to `Signal.Schedule`.
...
`Signal.Quarterly` SHALL pass the `day` parameter through to each `Schedule` instance. The day modes resolve at the **monthly** level within each quarterly month.

#### Scenario: Quarterly default (end) behavior unchanged
...
- **THEN** it fires 4 times — on the first trading day on or after the 10th of Feb, May, Aug, Nov
