...
The parameter `next_trading_day` SHALL be renamed to `closest_trading_day`. Default remains `True`. When `False`, the signal SHALL only fire if the exact target day is a NYSE trading day.
